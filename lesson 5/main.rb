...
class RailRoad
  ...
  # загоняем все под протектект, потом у что пока не предполагается использование экземпляров класса
  private
  ...
  def create_train
    ...
    return trains << PassengerTrain.new(train_number, :passenger) if train_type == 1

    trains << CargoTrain.new(train_number, :cargo)
  end
